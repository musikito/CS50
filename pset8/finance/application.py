...
def index():
    ...
    # select all the stocks owned by the user
    # added a "transaction" table to the DB to keep track of buy/sells
    stocks = db.execute(
    ...
    # show the current portfolio and remaining cash balance
...
def buy():
    ...
    if request.method == "POST":
        ...
        # TODO put this 2 in one function
        try:
            shares = int(request.form.get("shares"))
        except:
            return apology("shares must be a positive integer", 400)

        # Need to buy at least 1 share
        if shares <= 0:
        ...
        # How much cash/money the user still has in the account
...
def register():
    ...
    if request.method == "POST":
        ...
        # Redirect user to home page
        # need to expicit import url_for from Flask, see top imports
       # return redirect(url_for("index"))
        return redirect("/")
...
def sell():
    ...
    # This function is almost the same as the buy function
    ...
    if request.method == "POST":
        ...
        # This is "almost" the same as the buy function
        # need to find a way to combine this into a more efficient transaction
        db.execute("UPDATE users SET cash = cash + :price WHERE id = :user_id", price=total_price, user_id=session["user_id"])
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price_per_share) VALUES(:user_id, :symbol, :shares, :price)",
                   user_id=session["user_id"],
                   symbol=request.form.get("symbol"),
